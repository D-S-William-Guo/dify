...
function Ensure-EnterpriseImage {
  ...
  $buildArgs = @("--build-arg", "COMMIT_SHA=$ExpectedCommitSha")
  if ($Dockerfile -like "*web/Dockerfile" -or $Dockerfile -like "*web\Dockerfile") {
    if ($env:NPM_REGISTRY) {
      $buildArgs += @("--build-arg", "NPM_REGISTRY=$env:NPM_REGISTRY")
    }
  }
  docker build @buildArgs -f $Dockerfile -t $Image $ContextPath
}
