...
- 当前官方稳定发布基线：`1.16.1` tag/tree
- 当前企业候选分支：`codex/enterprise-candidate-1.16.1-20260728`
- 当前企业版本：`1.16.1-enterprise`
- 正式企业镜像：
...
本轮重建已核对官方 `1.16.1` tag/tree 的关键差异。企业候选必须遵守以下升级要求：

- 数据库：升级后运行 `flask db upgrade`，本地源码命令使用 `uv run --project api flask db upgrade`。
- Dify Agent：官方 `1.16.1` 包含 `agent_backend` 和 `local_sandbox` compose 服务，企业 overlay 必须保留这些官方服务，并确认其镜像解析为 `langgenius/dify-agent-backend:1.16.1` 和 `langgenius/dify-agent-local-sandbox:1.16.1`。
- Docker/env：平移旧 `.env` 后必须按 `docker/.env.example` 与 `docker/envs/**/*.env.example` 补齐 `DIFY_AGENT_*`、`AGENT_BACKEND_*`、Redis keepalive、`API_WEBSOCKET_WORKER_AMOUNT`、`WORKFLOW_GENERATION_TIMEOUT_MS`、Agent V2/feature preview 等 1.16 新配置。
- 插件自动升级策略：如果运行数据未经历过 `1.15.0` 的 `flask backfill-plugin-auto-upgrade`，数据库迁移后仍需补跑该命令；正常从 `1.15.0-enterprise` 升到 `1.16.1-enterprise` 时应已完成。
- Plugin daemon：官方已提供 `PIP_MIRROR_AUTO_DETECT` 与 `PIP_MIRROR_URL`。企业 overlay 优先复用官方实现，只保留离线部署所需的显式传递和文档说明。
- 安全修复：不得回退既有官方 plugin-daemon forwarding path traversal（CVE-2026-41948）修复。
- 重点回归：Dify Agent / Agent V2、workflow、dataset、plugin install/OAuth/cache、dataset/RAG 与 workflow execution/session 管理相关变更都需要在点击验证中覆盖。
...
head: D-S-William-Guo/dify:codex/enterprise-candidate-1.16.1-20260728
```
...
- `codex/enterprise-candidate-1.16.1-20260728`：当前最佳企业候选分支，基于官方 `1.16.1` tag/tree。
...
以后官方发布 `1.17.0`、`1.18.0` 等稳定版本时，固定流程是：
...
旧候选中的 DSL、configuration hook、workflow、plugin、dataset、tool 等广覆盖补丁，必须先对比官方 `1.16.1`，确认官方没有等价修复且企业能力仍需要后，才允许最小重放。
...
```powershell
git clone https://github.com/langgenius/dify.git D:\CodexSpace\dify-enterprise-1.16.1-candidate-20260728
cd D:\CodexSpace\dify-enterprise-1.16.1-candidate-20260728
git remote rename origin upstream
git remote add origin https://github.com/D-S-William-Guo/dify.git
git fetch upstream tag 1.16.1
git switch -c codex/enterprise-candidate-1.16.1-20260728 1.16.1
```

如果工作树已经存在，应先确认目录内容，不得删除未知文件。当前候选已从 `refs/tags/1.16.1` 创建。
...
- 启动新环境时必须显式传入 `DIFY_ENTERPRISE_VERSION=<new-version>-enterprise` 和需要的 `COMPOSE_PROFILES`。1.16.1 起默认应包含 `collaboration`，即常规本机验证使用 `COMPOSE_PROFILES=weaviate,postgresql,collaboration`，不要只依赖旧 `.env` 里的 profile 或镜像配置。启动前执行 `docker compose ... config --images`，确认 API/Web/worker/api_websocket 全部解析到当前企业版本镜像。
...
迁移完成后，应通过只读检查确认旧数据已经接管新环境，例如账户、空间、应用、知识库、已安装插件、智慧广场资产数量，以及 `alembic_version` 是否到达当前企业迁移 head（本轮为 `a7b8c9d0e1f2`）。浏览器访问 `/apps` 应进入登录页或已登录工作台，不应进入初始化页。
...
- 官方版本：`1.16.1`
- 企业版本：`1.16.1-enterprise`
- API 镜像：`dify-api-enterprise:1.16.1-enterprise`
- Web 镜像：`dify-web-enterprise:1.16.1-enterprise`

`worker` 与 `worker_beat` 运行时继续复用 `dify-api-enterprise:1.16.1-enterprise`。
