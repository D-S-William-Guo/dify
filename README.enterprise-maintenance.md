...
- 离线发布默认使用“已验证镜像包 + 最小配置包”两件套；配置包只包含 compose、nginx、ssrf、env 示例和清单文件，不包含 `docker/volumes/**`、源码构建缓存或本机运行产物
- 远程机器已有状态时，保留远程运行数据和 `.env`，先备份再用 `docker/dify-env-sync.py` 或 shell 版本合并新增环境变量，不要用本地目录整树覆盖远程部署
