...
- 当前官方稳定发布基线：`1.14.2` tag/tree
- 当前企业候选分支：`codex/enterprise-candidate-1.14.2-20260519`
- 当前企业版本：`1.14.2-enterprise`
- 正式企业镜像：
  - `dify-api-enterprise:1.14.2-enterprise`
  - `dify-web-enterprise:1.14.2-enterprise`
...
head: D-S-William-Guo/dify:codex/enterprise-candidate-1.14.2-20260519
```
...
- `codex/enterprise-candidate-1.14.2-20260519`：当前最佳企业候选分支，基于官方 `1.14.2` tag/tree。
...
旧候选中的 DSL、configuration hook、workflow、plugin、dataset、tool 等广覆盖补丁，必须先对比官方 `1.14.2`，确认官方没有等价修复且企业能力仍需要后，才允许最小重放。
...
```powershell
git clone https://github.com/langgenius/dify.git D:\CodexSpace\dify-enterprise-1.14.2-candidate-20260519
cd D:\CodexSpace\dify-enterprise-1.14.2-candidate-20260519
git remote rename origin upstream
git remote add origin https://github.com/D-S-William-Guo/dify.git
git fetch upstream tag 1.14.2
git switch -c codex/enterprise-candidate-1.14.2-20260519 1.14.2
```

如果工作树已经存在，应先确认目录内容，不得删除未知文件。当前候选已从 `refs/tags/1.14.2` 创建。
...
- 启动新环境时必须显式传入 `DIFY_ENTERPRISE_VERSION=<new-version>-enterprise` 和需要的 `COMPOSE_PROFILES`。1.14.2 起默认应包含 `collaboration`，即常规本机验证使用 `COMPOSE_PROFILES=weaviate,postgresql,collaboration`，不要只依赖旧 `.env` 里的 profile 或镜像配置。启动前执行 `docker compose ... config --images`，确认 API/Web/worker/api_websocket 全部解析到当前企业版本镜像。
...
- 官方版本：`1.14.2`
- 企业版本：`1.14.2-enterprise`
- API 镜像：`dify-api-enterprise:1.14.2-enterprise`
- Web 镜像：`dify-web-enterprise:1.14.2-enterprise`

`worker` 与 `worker_beat` 运行时继续复用 `dify-api-enterprise:1.14.2-enterprise`。
