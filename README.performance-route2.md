...
- 如果路线二改动与 `upstream/main` 当前前端构建基线冲突，先恢复到官方当前主线可构建的状态，再重新套用本文件中的治理规则

前端构建边界：

- 路线二是阶段性治理，不得覆盖 `upstream/main` 当前前端构建基线
- 遇到前端构建失败时，不要默认在企业分支旧方案上继续补丁
- 先分析 `upstream/main` 当前采用的构建方式，再让企业分支回到同一方向，然后才讨论哪些路线二规则继续保留
