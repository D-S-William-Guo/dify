...
def test_serialize_asset_list_should_pass_prefetched_models_to_serializer(mocker: MockerFixture) -> None:
...
def test_serialize_asset_list_should_skip_assets_without_normal_source_apps(mocker: MockerFixture) -> None:
...
def test_list_public_assets_should_filter_out_non_normal_source_apps_in_query(mocker: MockerFixture) -> None:
    mocked_db = mocker.patch("services.enterprise_marketplace_service.db")
    ...
    mocker.patch.object(EnterpriseMarketplaceService, "serialize_asset_list", return_value=[])
...
def test_list_admin_assets_should_filter_out_non_normal_source_apps_in_query(mocker: MockerFixture) -> None:
    mocked_db = mocker.patch("services.enterprise_marketplace_service.db")
    mocked_db.paginate.side_effect = lambda *, select, page, per_page, error_out: _capture_paginate_args(
        select_statement=select,
        page=page,
