...
def test_serialize_asset_list_should_skip_assets_without_normal_source_apps(monkeypatch: MonkeyPatch) -> None:
...
def test_list_public_assets_should_filter_out_non_normal_source_apps_in_query(monkeypatch: MonkeyPatch) -> None:
    mocked_db = Mock()
    monkeypatch.setattr("services.enterprise_marketplace_service.db", mocked_db)
...
def test_list_admin_assets_should_filter_out_non_normal_source_apps_in_query(monkeypatch: MonkeyPatch) -> None:
    mocked_db = Mock()
    monkeypatch.setattr("services.enterprise_marketplace_service.db", mocked_db)
    mocked_db.paginate.side_effect = lambda *, select, page, per_page, error_out: _capture_paginate_args(
        select_statement=select,
