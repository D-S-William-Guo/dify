...
Base: official stable tag/tree `1.14.0` at `6fafeec41502152ec3552d1851aaeb3c22db1f02`

Candidate branch: `codex/enterprise-candidate-1.14.0-20260430`

Enterprise version: `1.14.0-enterprise`
...
Rebuild the enterprise branch from official stable `1.14.0`, replaying only the patches required to preserve enterprise workspace, platform-admin, 智慧广场, Docker/offline packaging, plugin offline install, and dataset/hit-testing behavior while keeping official `1.14.0` as the default source of truth.

## Branch Truth

- `codex/enterprise-candidate-1.14.0-20260430`: current candidate and active work surface.
...
- Prefer official `1.14.0` implementation unless an enterprise capability would break.
...
- Enterprise API image builds as `dify-api-enterprise:1.14.0-enterprise`.
- Enterprise web image builds as `dify-web-enterprise:1.14.0-enterprise`.
...
Purpose: explicitly re-check previously fragile DSL and configuration areas against official `1.14.0` before carrying old hotfixes.
...
Purpose: carry only enterprise-required compatibility patches after official `1.14.0` behavior is understood.
...
- Old tests that only assert pre-`1.14.0` APIs
...
5. Re-check install/sign-in/public route behavior against official `1.14.0` first, then patch only proven regressions.
...
3. Confirm `api/pyproject.toml` and `web/package.json` both report `1.14.0`.
4. Rebuild enterprise `api` and `web` images as `1.14.0-enterprise`.
