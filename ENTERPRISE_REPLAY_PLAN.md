...
Base: official stable tag/tree `1.14.1` at `51a8f79d67de30f77f34d22105039aeb5526811f`

Candidate branch: `codex/enterprise-candidate-1.14.1-20260514`

Enterprise version: `1.14.1-enterprise`
...
Rebuild the enterprise branch from official stable `1.14.1`, replaying only the patches required to preserve enterprise workspace, platform-admin, 智慧广场, Docker/offline packaging, plugin offline install, and dataset/hit-testing behavior while keeping official `1.14.1` as the default source of truth.

## Branch Truth

- `codex/enterprise-candidate-1.14.1-20260514`: current candidate and active work surface.
...
- Prefer official `1.14.1` implementation unless an enterprise capability would break.
...
- Enterprise API image builds as `dify-api-enterprise:1.14.1-enterprise`.
- Enterprise web image builds as `dify-web-enterprise:1.14.1-enterprise`.
...
Purpose: explicitly re-check previously fragile DSL and configuration areas against official `1.14.1` before carrying old hotfixes.
...
Purpose: carry only enterprise-required compatibility patches after official `1.14.1` behavior is understood.
...
- Old tests that only assert pre-`1.14.1` APIs
...
5. Re-check install/sign-in/public route behavior against official `1.14.1` first, then patch only proven regressions.
...
3. Confirm `api/pyproject.toml` and `web/package.json` both report `1.14.1`.
4. Rebuild enterprise `api` and `web` images as `1.14.1-enterprise`.
