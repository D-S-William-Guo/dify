...
Base: official stable tag/tree `1.16.1` at `6f8ed69e`

Candidate branch: `codex/enterprise-candidate-1.16.1-20260728`

Enterprise version: `1.16.1-enterprise`

Status: enterprise patch set replayed onto the official stable release baseline. Validate the source tree, migrations, enterprise images, compose runtime, and offline package before promotion.

## Official 1.16.1 Upgrade Notes

- Run `flask db upgrade` after updating code. Local source validation should use `uv run --project api flask db upgrade`.
- Official `1.16.1` includes the Dify Agent backend and local sandbox compose services. Enterprise compose validation must keep those official services and verify they resolve to `langgenius/dify-agent-backend:1.16.1` and `langgenius/dify-agent-local-sandbox:1.16.1`.
- Review Docker/env changes before starting compose: the official `1.16.1` tag includes Agent backend variables, Redis keepalive variables, `API_WEBSOCKET_WORKER_AMOUNT`, `WORKFLOW_GENERATION_TIMEOUT_MS`, and Agent V2/feature-preview defaults.
- If upgrading data that has not already passed the `1.15.0` plugin auto-upgrade backfill, run `flask backfill-plugin-auto-upgrade` after the DB migration. A normal `1.15.0-enterprise` to `1.16.1-enterprise` upgrade should already have this backfill.
- Plugin daemon supports `PIP_MIRROR_AUTO_DETECT` and `PIP_MIRROR_URL`; enterprise offline plugin install fixes should reuse those official knobs instead of carrying a parallel mirror mechanism.
- Preserve prior official security fixes, especially plugin-daemon forwarding path traversal CVE-2026-41948.
- Re-test Dify Agent / Agent V2, workflow, dataset, plugin, and RAG paths touched by official `1.16.1`.

## Goal

Rebuild the enterprise branch from official stable `1.16.1`, replaying only the patches required to preserve enterprise workspace, platform-admin, 智慧广场, Docker/offline packaging, plugin offline install, and dataset/hit-testing behavior while keeping official `1.16.1` as the default source of truth.

## Branch Truth

- `codex/enterprise-candidate-1.16.1-20260728`: current candidate and active work surface.
...
- Prefer official `1.16.1` implementation unless an enterprise capability would break.
...
- `api/migrations/versions/2026_04_30_2100-c8f3d9d4a1be_add_enterprise_marketplace_assets.py`
...
- Enterprise API image builds as `dify-api-enterprise:1.16.1-enterprise`.
- Enterprise web image builds as `dify-web-enterprise:1.16.1-enterprise`.
- Compose services start without migration multiple-head errors. The enterprise migration head should be `a7b8c9d0e1f2`.
...
Purpose: explicitly re-check previously fragile DSL and configuration areas against official `1.16.1` before carrying old hotfixes.
...
Purpose: carry only enterprise-required compatibility patches after official `1.16.1` behavior is understood.
...
Additional patches replayed in this group for `1.16.1-enterprise`:
...
- Old tests that only assert pre-`1.16.1` APIs
...
5. Re-check install/sign-in/public route behavior against official `1.16.1` first, then patch only proven regressions.
...
3. Confirm `api/pyproject.toml` and `web/package.json` both report `1.16.1`.
...
6. Rebuild enterprise `api` and `web` images as `1.16.1-enterprise`.
...
8. Verify migrated data is present, including accounts, workspaces, apps/workflows, datasets, installed plugins, and enterprise marketplace assets when available. Confirm Alembic reaches enterprise head `a7b8c9d0e1f2`.
