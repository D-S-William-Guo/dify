...
Base: official stable tag/tree `1.14.2` at `7f392b6`

Candidate branch: `codex/enterprise-candidate-1.14.2-20260519`
...
Rebuild the enterprise branch from official stable `1.14.2`, replaying only the patches required to preserve enterprise workspace, platform-admin, 智慧广场, Docker/offline packaging, plugin offline install, and dataset/hit-testing behavior while keeping official `1.14.2` as the default source of truth.

## Branch Truth

- `codex/enterprise-candidate-1.14.2-20260519`: current candidate and active work surface.
...
- Prefer official `1.14.2` implementation unless an enterprise capability would break.
...
- Enterprise API image builds as `dify-api-enterprise:1.14.2-enterprise`.
- Enterprise web image builds as `dify-web-enterprise:1.14.2-enterprise`.
...
Purpose: explicitly re-check previously fragile DSL and configuration areas against official `1.14.2` before carrying old hotfixes.
...
Purpose: carry only enterprise-required compatibility patches after official `1.14.2` behavior is understood.
...
- Old tests that only assert pre-`1.14.2` APIs
...
5. Re-check install/sign-in/public route behavior against official `1.14.2` first, then patch only proven regressions.
...
3. Confirm `api/pyproject.toml` and `web/package.json` both report `1.14.2`.
4. For local upgrade validation, stop compose, migrate the previous stable enterprise `docker/.env` and `docker/volumes/**` into the new worktree, then start compose from the new worktree only.
...
6. Rebuild enterprise `api` and `web` images as `1.14.2-enterprise`.
