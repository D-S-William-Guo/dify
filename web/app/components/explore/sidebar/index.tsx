...
const SideBar = () => {
  ...
  const isMarketplaceSelected = lastSegment === 'marketplace'
  ...
  return (
    <div className={cn('flex h-full w-fit shrink-0 cursor-pointer flex-col px-3 pt-6 sm:w-[240px]', isFold && 'sm:w-[56px]')}>
      ...
      <div className={cn('mt-1', isMarketplaceSelected ? 'text-text-accent' : 'text-text-tertiary')}>
        <Link
          href="/explore/marketplace"
          aria-label={isMobile || isFold ? t('enterpriseMarketplace.sidebarTitle', { ns: 'common' }) : undefined}
          className={cn(isMarketplaceSelected ? 'bg-state-base-active' : 'hover:bg-state-base-hover', 'flex h-8 items-center gap-2 rounded-lg px-1 mobile:w-fit mobile:justify-center pc:w-full pc:justify-start')}
        >
          <div className="flex size-6 shrink-0 items-center justify-center rounded-md bg-components-icon-bg-blue-solid">
            <span aria-hidden="true" className="i-ri-store-2-fill size-3.5 text-components-avatar-shape-fill-stop-100" />
          </div>
          {!isMobile && !isFold && <div className={cn('truncate', isMarketplaceSelected ? 'system-sm-semibold text-components-menu-item-text-active' : 'system-sm-regular text-components-menu-item-text')}>{t('enterpriseMarketplace.sidebarTitle', { ns: 'common' })}</div>}
        </Link>
      </div>
