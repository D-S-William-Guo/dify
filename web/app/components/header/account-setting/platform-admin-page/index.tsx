...
const InviteMembersDialog = ({
  ...
  const getRoleLabel = (role: PlatformAdminRoleKey) => {
    switch (role) {
      case 'admin':
        return t(($) => $["members.admin"], { ns: 'common', defaultValue: role })
      case 'editor':
        return t(($) => $["members.editor"], { ns: 'common', defaultValue: role })
      case 'normal':
        return t(($) => $["members.normal"], { ns: 'common', defaultValue: role })
      case 'dataset_operator':
        return t(($) => $["members.datasetOperator"], { ns: 'common', defaultValue: role })
    }
  }
...
const PlatformAdminPage = () => {
  ...
  const getMemberRoleLabel = (role: Member['role']) => {
    switch (role) {
      case 'owner':
        return t(($) => $["members.owner"], { ns: 'common', defaultValue: role })
      case 'admin':
        return t(($) => $["members.admin"], { ns: 'common', defaultValue: role })
      case 'editor':
        return t(($) => $["members.editor"], { ns: 'common', defaultValue: role })
                        ...
                        <div className="w-[160px] shrink-0">
                          {member.role === 'owner'
                            ...
                            : (
                                ...
                                >
                                  {roleOptions.map(option => (
                                    <option key={option} value={option}>{getMemberRoleLabel(option)}</option>
                                  ))}
                                </select>
                              )}
                        </div>
