...
- `codex/enterprise-candidate-1.14.2-20260519` is the current clean enterprise candidate.
...
- Official version: `1.14.2`
- Enterprise version: `1.14.2-enterprise`
- API image: `dify-api-enterprise:1.14.2-enterprise`
- Web image: `dify-web-enterprise:1.14.2-enterprise`
...
```powershell
$env:DIFY_ENTERPRISE_VERSION = "1.14.2-enterprise"
```
...
- After rebuilding `dify-api-enterprise:1.14.2-enterprise`, recreate `api`, `api_websocket`, `worker`, `worker_beat`, and `nginx`:
...
- After rebuilding `dify-web-enterprise:1.14.2-enterprise`, recreate `web` and `nginx`:
...
`api_websocket` is part of the 1.14.2 collaboration runtime and must be covered by the enterprise overlay so it uses the same enterprise API image as `api`.
...
```bash
export DIFY_ENTERPRISE_VERSION=1.14.2-enterprise
...
Do not rebuild during final packaging after runtime validation. `Mode=reuse` should fail if `dify-api-enterprise:1.14.2-enterprise` or `dify-web-enterprise:1.14.2-enterprise` is missing or inconsistent.
