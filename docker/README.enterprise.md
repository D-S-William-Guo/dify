...
- Official stable tag/tree `1.16.1` is the current enterprise release baseline.
- `codex/enterprise-candidate-1.16.1-20260728` is the current clean enterprise candidate.
...
## Official 1.16.1 Docker and Env Notes

- Run `flask db upgrade` after startup/migration.
- Official `1.16.1` includes `agent_backend` and `local_sandbox` services. Keep them from the official compose file and verify they resolve to `langgenius/dify-agent-backend:1.16.1` and `langgenius/dify-agent-local-sandbox:1.16.1`.
- Review official env changes before reusing an older `.env`: copy forward `DIFY_AGENT_*`, `AGENT_BACKEND_*`, Redis keepalive, `API_WEBSOCKET_WORKER_AMOUNT`, `WORKFLOW_GENERATION_TIMEOUT_MS`, and Agent V2/feature-preview settings from the new examples.
- If the deployment is upgrading from a pre-`1.15.0` runtime or skipped the previous plugin auto-upgrade migration, run `flask backfill-plugin-auto-upgrade` after `flask db upgrade`.
...
- Preserve the official plugin-daemon path traversal fix; do not reuse older plugin-daemon forwarding behavior from enterprise history.
...
- Official version: `1.16.1`
- Enterprise version: `1.16.1-enterprise`
- API image: `dify-api-enterprise:1.16.1-enterprise`
- Web image: `dify-web-enterprise:1.16.1-enterprise`
...
```powershell
$env:DIFY_ENTERPRISE_VERSION = "1.16.1-enterprise"
```
...
$env:NPM_REGISTRY = "https://registry.npmmirror.com" # Optional: use when npmjs.org is slow or unreachable.
...
- After rebuilding `dify-api-enterprise:1.16.1-enterprise`, recreate `api`, `api_websocket`, `worker`, `worker_beat`, and `nginx`:
...
- After rebuilding `dify-web-enterprise:1.16.1-enterprise`, recreate `web` and `nginx`:
...
`api_websocket` is part of the 1.16.1 collaboration runtime and must be covered by the enterprise overlay so it uses the same enterprise API image as `api`.
...
The expected result is that API/Web/worker containers use the current enterprise image tag and image IDs, the official Agent containers use the `1.16.1` Agent images, every bind mount points to the new worktree, and the migrated database reaches the current enterprise Alembic head (`a7b8c9d0e1f2`). Old enterprise images may remain on disk as cache, but no running container may reference an old enterprise tag or old worktree mount.
...
Do not rebuild during final packaging after runtime validation. `Mode=reuse` should fail if `dify-api-enterprise:1.16.1-enterprise` or `dify-web-enterprise:1.16.1-enterprise` is missing or inconsistent.
...
- `docker/UPGRADE_1.15.0_TO_1.16.1_ENTERPRISE.md`
