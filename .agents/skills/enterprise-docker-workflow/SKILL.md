...
- For `1.16.1`, run `flask db upgrade`. If the runtime data has not already passed the `1.15.0` plugin auto-upgrade backfill, also run `flask backfill-plugin-auto-upgrade`.
- Keep official `1.16.1` Agent services (`agent_backend` and `local_sandbox`) in compose validation; they should resolve to `langgenius/dify-agent-backend:1.16.1` and `langgenius/dify-agent-local-sandbox:1.16.1`.
