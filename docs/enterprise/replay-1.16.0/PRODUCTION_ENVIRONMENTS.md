...
1. 本轮基线固定为 **1.16.0**。现有主机上 1.16.1 是同事基于 1.16.0 + 官方 1.16.1 安全增强手工改的，规范度不如本轮；本轮方法验证通过后，再用同一套方法验证 1.16.1。
2. 现有 `docker-1161-*` 是同事部署，**保留不动**，作为现有运行基线/Plan A。
3. 本轮部署按我们自己的标准化方式部署为 **Plan B/后盾**，保留同事的部署习惯（命名/端口/目录习惯尽量兼容），不与现有栈冲突。
4. `dify-api-expand:latest` 是团队自研 Dify 安全围栏服务，**本轮不触碰、不迁移、不纳入离线包**。
